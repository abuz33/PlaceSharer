...
const createReview = async (req, res, next) => {
	...
	if (!errors.isEmpty()) {
		return next(new HttpError('Invalid review inputs passed, please check your data.', 422));
	}

	const { date, reviewTxt, creator, image, place } = req.body;
...
const getReviewsByPlaceId = async (req, res, next) => {
	...
	try {
		placeWithReviews = await Review.find({place: placeId});
	} catch (err) {
